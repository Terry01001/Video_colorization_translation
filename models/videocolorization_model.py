...
class VideoColorizationModel(Pix2PixModel):
    ...
    def get_image_paths(self, frame_number):
        video_name = self.opt.video_name.split('.')[0]
        os.makedirs(os.path.join(self.opt.results_dir, self.opt.name, video_name), exist_ok=True)
        return os.path.join(self.opt.results_dir, self.opt.name, video_name, f'{frame_number:05d}.png')
